...
const Esculturas = () => {
    ...
    // Abrir y cerrar el modal de compartir
    ...
    // Función para obtener imágenes por escultor
    const fetchImagenes = async (id_escultor) => {
        try {
            const response = await axios.get(`http://localhost:3000/img_escultura/${id_escultor}`);
            console.log("DATOS", response.data);
            setImage(""); // Asegúrate de que `response.data` contenga lo esperado
        } catch (err) {
            console.error(err);
    ...
    // Manejo de la navegación al perfil del escultor
    const handleEscultor = (escultura) => {
        navigate(`/biografia/${escultura.id_escultor}`);
        fetchImagenes(escultura.id_escultor); // Llama a la función para obtener imágenes
    };
                            ...
                            <div className="card-body">
                                <div className="image-container">
                                    <img
                                        ...
                                        src={image} // Aquí asegúrate de que `image` esté correctamente definido
